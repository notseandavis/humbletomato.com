...
- **No AI writing tells.** Avoid: em dashes (—), "The Core Insight," "Let's dive in," "Here's the thing," "It's worth noting," "Crucially," "Importantly," "Why X are..." (subtitle pattern). Use periods and commas. Write like a person, not a language model. If a heading sounds like a ChatGPT section title, rewrite it. Headings should sound human, not robotic. Check subtitles especially — "Why specifications are genetic code" sounds like AI, "Specs encode intent like DNA encodes organisms" sounds human. "Don't Pre-Synthesize Your Specs" sounds like a bot; "Feed Raw Context" sounds human.
...
*Last updated: 2026-03-24 — Improved fuzzy-compiler.html with multi-currency production failure example showing silent errors that surface in production (not just compile-time ambiguity). Specs-as-dna.html information theory section could still use more code grounding. Gold standards: solution-space.html (table-driven constraints), ralph-wiggum.html (format → examples → acceptance), spec-driven-development.html (portable specs), matrix-methodology.html (ASCII + callouts), code-diffusion-model.html (spec progression), fuzzy-compiler.html (silent failure pattern). Research: Spec-driven development now mainstream (66% cite "almost right" as biggest AI frustration). Agent harness frameworks maturing (superpowers +38K stars/mo, gstack 23K week one). 85% developers use AI tools regularly. 535% growth in AI agent framework repos 2024-2025.*
