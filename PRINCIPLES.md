...
*Last updated: 2026-03-27 — **All pages now meet 3-screen rule** (under 120 lines). specs-as-dna.html cut from 152→~100 lines by removing biology table and merging sections. Gold standards: solution-space, ralph-wiggum, spec-driven-development, matrix-methodology, code-diffusion-model, fuzzy-compiler, vibe-coding-vs-spec-driven, matrix-way-of-working (8/17 pages exemplary). Remaining minor improvements: telephone-game should lead with code (currently starts with callout), what-the-model-knows could use more concrete examples. Agent orchestration trending: K8s-native (Optio), multi-agent for Claude Code (oh-my-claudecode), long-horizon enterprise agents (deer-flow from ByteDance). Framework ecosystem maturing rapidly.*
